...
// ✅ keep real react-router-dom exports, only override Link
...
describe("Products page", () => {
  ...
  test("fetches products on mount and renders product cards with correct links + images", async () => {
    const products = [
      { _id: "p1", slug: "prod-1", name: "Product 1", description: "Desc 1" },
      { _id: "p2", slug: "prod-2", name: "Product 2", description: "Desc 2" },
    ];
    axios.get.mockResolvedValueOnce({ data: { products } });

    render(<Products />);

    // ✅ wait for UI after state update (not just axios call)
